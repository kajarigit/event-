...
export default function Stalls() {
  ...
  const [stallCredentials, setStallCredentials] = useState(null);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      // Show success message with stall owner credentials
      if (stallData?.temporaryPassword && stallData?.ownerEmail) {
        toast.success(
          <div className="space-y-2">
            <div className="font-bold text-green-800">✅ Stall Created Successfully!</div>
            <div className="text-sm">"{stallData?.name || 'Stall'}" has been added to the event.</div>
            <div className="bg-green-50 p-2 rounded border border-green-200 space-y-1">
              <div className="font-semibold text-green-800 text-xs">🔐 Stall Owner Login Credentials:</div>
              <div className="text-xs font-mono bg-white p-1 rounded border">
                📧 Email: {stallData.ownerEmail}
              </div>
              <div className="text-xs font-mono bg-white p-1 rounded border">
                🔑 Password: {stallData.temporaryPassword}
              </div>
        ...
        // Also show a persistent notification that can be copied
        toast(
          <div className="space-y-2">
            <div className="font-bold text-blue-800">📋 Credentials for "{stallData?.name}"</div>
            <div className="bg-blue-50 p-2 rounded text-xs space-y-1">
              <div>Email: <span className="font-mono bg-white px-1 rounded">{stallData.ownerEmail}</span></div>
              <div>Password: <span className="font-mono bg-white px-1 rounded">{stallData.temporaryPassword}</span></div>
              <button
                onClick={() => {
                  navigator.clipboard.writeText(`Stall Owner Login:\nEmail: ${stallData.ownerEmail}\nPassword: ${stallData.temporaryPassword}\nLogin URL: ${window.location.origin}/stall-owner/login`);
                  toast.success('📋 Credentials copied to clipboard!', { duration: 2000 });
                }}
          ...
          {
            duration: 20000, // Very long duration for easy access
            style: {
              background: '#EFF6FF',
              color: '#1E40AF',
        ...
        // Fallback for stalls without owner credentials
        toast.success(
          <div>
      ...
      if (stallData?.temporaryPassword && stallData?.ownerEmail) {
        setStallCredentials({
          ...
          ownerEmail: stallData.ownerEmail,
          password: stallData.temporaryPassword,
          loginUrl: stallData.dashboardLoginUrl || `${window.location.origin}/stall-owner/login`
        });
...
REPLACE_WITH_EVENT_UUID,IoT Solutions,Smart devices and IoT,Block B - Room 202,Technology,Prof. Neha Singh,9876543211,neha.singh@college.edu,iot2024pass,Electronics,"[{\\"name\\":\\"Vikram Reddy\\",\\"regNo\\":\\"2024EC012\\",\\"department\\":\\"Electronics\\"}]"`;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stall Owner Credentials Modal */}
      {showCredentialsModal && stallCredentials && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-lg w-full">
            <div className="flex justify-between items-center p-6 border-b">
              <h3 className="text-xl font-bold text-green-600">🎉 Stall Created Successfully!</h3>
              <button
            ...
            <div className="p-6 space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-sm font-medium text-gray-700">🔐 Stall Owner Login Credentials:</label>
                    <div className="mt-2 space-y-2">
                      <div className="bg-white p-3 rounded border border-green-200">
                        <div className="text-sm text-gray-600">Email:</div>
                        <div className="font-mono text-sm font-medium bg-gray-50 px-2 py-1 rounded">
                          {stallCredentials.ownerEmail}
                        </div>
                      </div>
                      <div className="bg-white p-3 rounded border border-green-200">
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded p-3">
                    <div className="text-sm text-gray-700">
                      ...
                      <div className="font-mono text-xs break-all">{stallCredentials.loginUrl}</div>
                    </div>
                  </div>
                </div>
              </div>
...
Please save these credentials securely.`;
                ...
                >
                  <span>📋</span>
                  <span>{stallCredentials.password === '••••••••' ? 'Copy Info' : 'Copy All Credentials'}</span>
                </button>
                
                <button
                  onClick={() => {
                    const subject = encodeURIComponent(`Stall Owner Credentials - ${stallCredentials.stallName}`);
                    const body = encodeURIComponent(`Dear Stall Owner,

Your stall "${stallCredentials.stallName}" has been created successfully!
...
Please save these credentials securely and access your stall dashboard to manage your stall.
